...
var RSA = (function(){
    ...
    var _RSA = {
        _genRandomValue : function(){
            var key = Math.ceil(Math.random() * 255);
            return key;
        },
        _transTimeSeed : function(tm){
            var len = tm.length;
            var tmp = "";

            for(var i = 0; i < len; i += 2){
                tmp += String.fromCharCode(parseInt(tm.substr(i, 2), 16));
            }

            return tmp;
        },
        _encrypt2 : function(key, s){ //˳��
            var a = new Array();
            var sl = s.length;
            var i = 0;
            var cs = key.chunkSize;
        ...
        encrypt1 : function(key, s, tm, mac){
            var seed = _RSA._transTimeSeed(tm);
            var s1 = seed + s;

            if(true === mac){
                s1 = seed + "00000000000000" + s;
            }
            //console.info(s1)
            return _RSA._encrypt1(key, s1).toUpperCase();
        },
        encrypt2 : function(key, s, tm, mac){
            var seed = _RSA._transTimeSeed(tm);
            var s1 = seed + s;
            ...
            //console.info(s1)
            return _RSA._encrypt2(key, s1).toUpperCase();
        }
    };
    ...
    return {
        ...
        encrypt1 : _RSA.encrypt1,//����
        encrypt2 : _RSA.encrypt2,//˳��
        encrypt : _RSA._encrypt1,
        getTimeSeed : _RSA._transTimeSeed
    }
})();
